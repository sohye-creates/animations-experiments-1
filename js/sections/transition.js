// 섹션: TRANSITION — 하얀(First Section) 페이지가 고정된 채,
...
const FRAG = /* glsl */`
  ...
  uniform sampler2D tText1, tText2;
  uniform vec2 uRes;
  uniform float uProgress, uT1Aspect, uT2Aspect;
  uniform float uPixel, uScatter, uCurtainEnd, uContentStart;
  ...
  void main(){
    ...
    // 페이지1: 하얀 배경 + "First Section"(검정 글자)
    vec3 page1 = vec3(1.0);
    page1 = mix(page1, vec3(0.0), textA(tText1, c, uT1Aspect));
...
export function createTransition() {
  const params = {
    pixel: 20, scatter: 0.22, curtainEnd: 0.7, contentStart: 0.82,
  ...
  const material = new THREE.RawShaderMaterial({
    ...
    uniforms: {
      tText1: { value: t1.tex }, tText2: { value: t2.tex },
      uT1Aspect: { value: t1.aspect }, uT2Aspect: { value: t2.aspect },
      ...
      uC0: { value: new THREE.Color(params.c0) }, uC1: { value: new THREE.Color(params.c1) },
      uC2: { value: new THREE.Color(params.c2) }, uC3: { value: new THREE.Color(params.c3) },
    },
  });
  ...
  return {
    ...
    render(now, s) {
      ...
      u.uProgress.value = p; u.uRes.value.set(innerWidth, innerHeight);
      ...
      u.uC0.value.set(params.c0); u.uC1.value.set(params.c1);
      u.uC2.value.set(params.c2); u.uC3.value.set(params.c3);
